 ...
 * @setting         {Function}             [afterFirst]                A function that will be called once the first scan is done
 * @setting         {Boolean}              [scopes=true]              If true, the selector will be searched inside elements with the "s-scope" attribute as scopes
 * @setting         {Boolean}              [firstOnly=false]          If true, the query will stop after the first matching node is found
 * @setting         {TWhenTrigger|string}  [when]                      An optional when trigger or array of triggers to wait for before calling the callback with the detected node
 * @setting         {Function}             [disconnectedCallback]     An optional callback function that will be called when a previously detected node is removed from the dom
